...
RUN pip install --no-cache-dir --upgrade pip && pip install --no-cache-dir nvidia-cuda-runtime-cu12 vllm==0.15.0 runpod huggingface_hub pillow requests python-magic && apt-get clean && rm -rf /root/.cache
